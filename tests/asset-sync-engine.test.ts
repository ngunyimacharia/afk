...
test('discovers, validates, and classifies assets', async () => {
  ...
  const engine = new AssetSyncEngine({
    id: 'test',
    assetCategories: () => [{ name: 'docs', sourceRoot: src, destinationRoot: dst, extensions: ['.md'], validateSource: async (p) => { if (!p.endsWith('.md')) throw new Error('bad'); } }],
  });
...
test('creates missing destination directories', async () => {
  ...
  const engine = new AssetSyncEngine({ id: 'test', assetCategories: () => [{ name: 'docs', sourceRoot: src, destinationRoot: nestedDst, extensions: ['.md'] }] });
  const report = await engine.execute();
