...
import { createAfkOpencodeWith, extractModelsFromProvidersPayload, extractSessionOutput, extractSessionOutputLines, formatOpenCodeEvent, parseOpenCodeEvent, SDKOpenCodeSessionExecutor } from '../src/opencode.js';
...
test('extracts reviewer text from alternate message shapes', () => {
  assert.deepEqual(extractSessionOutput({
    messages: [
...
test('recovers stale opencode prompts in the same session', async () => {
  ...
  let createCalls = 0;
  let abortCalls = 0;
  const executor = new SDKOpenCodeSessionExecutor(async () => ({
  ...
  const executor = new SDKOpenCodeSessionExecutor(async () => ({
    ...
    client: {
      session: {
        ...
        messages: async () => [
          { info: { role: 'assistant' }, parts: [{ type: 'text', text: 'Partial work' }] },
        ],
      },
    },
  }) as never);
