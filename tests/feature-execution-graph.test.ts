...
test('rejects missing dependency references', () => {
  ...
  writeFileSync(path.join(issuesDir, '01.md'), '---\nfeature: feat\nstatus: ready-for-agent\nDepends-On:\n  - 99\n---\n');
  const repository = new TicketRepository(repoRoot);
  assert.throws(() => buildFeatureExecutionGraph(repoRoot, 'feat', repository.discoverTickets(), false), /missing dependency/);
});
...
test('writes execution json when persisted', () => {
  ...
  const execution = JSON.parse(readFileSync(path.join(repoRoot, '.scratch', 'feat', 'execution.json'), 'utf8')) as { feature: string; version: number };
