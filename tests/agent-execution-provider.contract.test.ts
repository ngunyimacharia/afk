...
test('pull-request permission policy rejects source edits and scratch writes', async () => {
  ...
  assert.equal(
    await decideAfkPermission(
      {
        sessionId: 'session-42',
        permissionId: 'per_bash_git_c_push',
        type: 'bash',
        title: 'bash',
        patterns: ['git -C /repo/.worktree/feature push --set-upstream origin HEAD'],
      },
      { policy },
    ),
    'always',
  ...
  assert.equal(
    await decideAfkPermission(
      {
        sessionId: 'session-42',
        permissionId: 'per_bash_gh_repo_pr_create',
        type: 'bash',
        title: 'bash',
        patterns: ['gh --repo owner/repo pr create --fill'],
      },
      { policy },
    ),
    'always',
  ...
  assert.equal(
    await decideAfkPermission(
      {
        sessionId: 'session-42',
        permissionId: 'per_bash_gh_pr_create_title',
        type: 'bash',
...
test('pull-request policy blocks workspace and scratch mutation', () => {
  ...
  assert.deepEqual(pullRequestPolicy.allowedCommandKinds, ['read', 'diagnostic', 'git-push', 'github-pr']);
  ...
  assert.equal(isCommandAllowed(pullRequestPolicy, { kind: 'diagnostic' }), true);
  ...
  assert.doesNotThrow(() => assertCommandAllowed(pullRequestPolicy, { kind: 'github-pr', target: 'gh pr create' }));
  assert.throws(
    () => assertCommandAllowed(pullRequestPolicy, { kind: 'write', target: 'src/app.ts' }),
    /Pull-request mode/,
  );
});
