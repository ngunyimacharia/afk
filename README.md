...
- the launch wizard always creates a GitHub pull request for each completed feature branch. There is no merge-back path and no "leave branches for manual inspection" path.
- the feature completion action is persisted as a launch preference. Legacy `mergeBackToBase` values resolve to `create-pr`.
