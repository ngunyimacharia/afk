...
function parseCommaSeparatedFlag(flag: string): string[] {
...
async function runHeadlessLaunch(input: HeadlessLaunchInput): Promise<{ code: number; message: string }> {
  ...
  const harnessFlag = parseStringFlag('--harness') as string;
  const modelFlag = parseStringFlag('--model') as string;
  const reviewerHarnessFlag = parseStringFlag('--reviewer-harness') as string;
  const reviewerModelFlag = parseStringFlag('--reviewer-model') as string;
  const featureSlugs = parseCommaSeparatedFlag('--features');
  const concurrencyFlag = parseStringFlag('--concurrency') as string;
  const completionFlag = parseStringFlag('--completion') as string;
  const sandboxFlag = parseStringFlag('--sandbox') as string;
